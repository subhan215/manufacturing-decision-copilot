...
- `docs/slides.pdf` — ten-slide deck, 16:9, rendered from `docs/slides.html` with headless Chrome. Ends on the limitations slide rather than a thank-you. Palette is lifted from the product's own tokens so deck and application read as one system, and each slide is set as a controlled document page carrying the real requirements hash.
